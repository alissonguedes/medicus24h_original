		...
		<tr class="{{ $row->status === '0' ? 'blocked' : null }}" style="position: relative;" id="{{ $row->id }}" data-disabled="{{ $disabled ? 'true' : 'false' }}">
			...
			<td data-disabled="true" width="15%" class="center-align">
				...
				@if(!$disabled)
					...
					<button class="btn-small btn-flat btn-edit btn-floating waves-effect" name="status" value="{{ $status }}" data-tooltip="{{ $status === '0' ? 'Bloquear' : 'Desbloquear' }}" data-link="{{ go(inica.pacientes.patch', 'status', $row->id) }}" data-method="patch">
