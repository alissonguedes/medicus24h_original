...
@if($paginate->total() > 0)
	...
	@foreach($paginate as $ind => $row)
		...
		<tr class="{{ $row->status === '0' ? 'blocked' : null }}" style="position: relative;" id="{{ $row->id }}" data-trigger="sidenav" data-url="{{ go('clinica.atendimentos.detalhes', $row->id) }}" data-target="atendimento" data-disabled="false">
			...
			<td>
				{? $status = ['0'=>'Não compareceu', '1'=>'Agendado', 'A'=>'Aguardando', 'I'=>'Iniciado', 'F'=>'Finalizado', 'R'=> 'Remarcado', 'C'=>'Cancelado']; ?}
			...
			<td data-disabled="true" width="18%" class="center-align border-rl">
				...
				@if(!$disabled)
					...
					<button type="button" class="btn-small ml-3 mr-3 teal lighten-3 btn-edit btn-floating waves-effect" name="status" value="{{ $status }}" data-trigger="sidenav" data-url="{{ go('clinica.atendimentos.detalhes', $row->id) }}" data-target="atendimento" data-tooltip="{{ $status === '0' ? 'Bloquear' : 'Desbloquear' }}" data-method="patch">
						<i class="material-icons white-text">{{ $row->status === '1' ? 'lock' : 'lock_open' }}</i>
					</button>
				@endif
				@if(!$disabled)
					<button type="button" class="btn-small red lighten-3 btn-floating excluir waves-effect" value="{{ $row->id }}" data-link="go(" data-tooltip="Excluir" data-method="delete">
